...
public class MonkeyInTheMiddlePartTwo {

    private static int COMMON_NUMBER = 0;
    public static void main(String[] args) {
        List<Monkey> monkeys = getActualData();
        COMMON_NUMBER = monkeys.stream().map(monkey -> monkey.testNumber).reduce((a,b)->a*b).get();
        int t=10000;
        while (t-->0)
            monkeys.forEach(monkey -> monkey.throwItems(monkeys));
        List<Long> inspectList = monkeys.stream().map(monkey -> monkey.inspectTimes).sorted(Comparator.reverseOrder()).collect(Collectors.toList());
    ...
    static class Monkey {
        ...
        public void throwItems(List<Monkey> monkeys) {
            while(!this.items.isEmpty()) {
                long worryLevel = this.items.poll();
                inspectTimes++;
                ...
                worryLevel = worryLevel % COMMON_NUMBER;
